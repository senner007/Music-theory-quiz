...
export abstract class AudiateQuizBase<T> extends AudioQuizBase<T> {
  ...
  abstract randomKey: TNoteSingleAccidental;
  ...
  async call_quiz(): Promise<string | never> {
    const solfege = new SolfegeMelody(this.audio().filter(a => a.display).first_and_only().audio as INotePlay[], this.randomKey, this.timeSignature);
